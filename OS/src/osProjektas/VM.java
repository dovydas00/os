...
/*	*//**
	...
	*//**
	...
	public boolean interpretCommand() {
		 ...
		 * processor.Halt(); interpretNext = false; } else if (
		 * command.startsWith( "SR" ) ) { processor.storeRegister( number );
		 * processor.incIC(); } else if ( command.startsWith( "CGRN" ) ) {
		 * processor.changeRN(); processor.incIC(); } else if (
		 * command.startsWith( "AD" ) ) { processor.addToRegister( number );
		 * processor.incIC(); } else if ( command.startsWith( "SB" ) ) {
		 * processor.substractFromRegister( number ); processor.incIC(); } else
		 * if ( command.startsWith( "CR" ) ) { processor.compareToRegister(
		 * number ); processor.incIC(); } else if ( command.startsWith( "JG" ) )
		 * { processor.jumpIfGreater( number ); } else if ( command.startsWith(
		 * "JE" ) ) { processor.jumpIfEqual( number ); } else if (
		 * command.startsWith( "JL" ) ) { processor.jumpIfLess( number ); } else
		 * if ( command.startsWith( "JP" ) ) { processor.jump( number ); } else
		 * if ( command.startsWith( "GW" ) ) { processor.getWord( number ); }
		 * else if ( command.startsWith( "GD" ) ) { processor.getData( number );
		 * } else if ( command.startsWith( "PW" ) ) { processor.printWord(
		 * number ); } else if ( command.startsWith( "PS" ) ) {
		 * processor.printString( number ); } else if ( command.startsWith( "LR"
		 * ) ) { processor.loadRegister( number ); processor.incIC(); } else {
		 * processor.setPI( 2 ); }
